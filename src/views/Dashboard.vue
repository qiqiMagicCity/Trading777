...
function updateTime() {
  const now = new Date();
  tz.value.NewYork = '纽约 (UTC-4): ' + now.toLocaleString('en-US', { timeZone:'America/New_York', hour12:false, year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit' });
  tz.value.Valencia = '瓦伦西亚 (UTC+2): ' + now.toLocaleString('en-ES', { timeZone:'Europe/Madrid', hour12:false, year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit' });
  tz.value.Shanghai = '上海 (UTC+8): ' + now.toLocaleString('zh-CN', { timeZone:'Asia/Shanghai', hour12:false, year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit' });
}
...
async function loadData() {
  ...
  const dailyPL = todays.reduce((s,t)=> s + t.price*t.quantity * (['Sell','Cover'].includes(t.type)?1:-1), 0);
  const unreal = 0;
  const dailyCount = todays.length;
  const { count: totalCount } = await supabase.from('trades').select('*',{ head:true, count:'exact' });
  const profitSymsArr = positions.filter(p=> (p.qty*(0-p.cost_per_share))>0 ).map(p=>p.symbol);
  const lossSymsArr = positions.filter(p=> (p.qty*(0-p.cost_per_share))<0 ).map(p=>p.symbol);
  const mtd=0, ytd=0;

  metricsList.value = [
    { key:'totalCost', title:'账户总持仓成本', value: totalCost, display:'$'+fmt(totalCost) },
    ...
    { key:'mtd', title:'本月盈利总和', value: mtd, display:'$'+fmt(mtd) },
    { key:'ytd', title:'本年盈利总和', value: ytd, display:'$'+fmt(ytd) },
  ];

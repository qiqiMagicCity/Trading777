...
<template>
  <div class="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
    <div v-for="k in kpis" :key="k.key" class="bg-white p-4 rounded-2xl shadow flex flex-col">
...
const { data } = useSWRV('kpi', fetcher, { refreshInterval: 10000 })
watchEffect(()=>{
  if(data.value){
    kpis.value.forEach(k=>k.value=data.value[k.key]||0)
  }
})

function format(n){ return (+n||0).toFixed(2) }
function valueColor(n){ return n>0?'text-emerald-500':n<0?'text-red-500':'text-gray-600' }
</script>

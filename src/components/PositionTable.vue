...
<table class="min-w-full divide-y divide-gray-200">
  <thead>
    <tr class="bg-gray-50 text-xs text-gray-500 uppercase">
      <th class="px-3 py-2">代码/股数</th><th class="px-3 py-2">实时价</th><th class="px-3 py-2">总成本</th>
      <th class="px-3 py-2">盈亏平衡</th><th class="px-3 py-2">持仓成本</th><th class="px-3 py-2">当日涨跌</th>
    </tr>
  </thead>
  <tbody class="divide-y divide-gray-200">
    <tr v-for="p in positions" :key="p.symbol">
      <td class="px-3 py-1 flex items-center space-x-2">
        <img :src="p.logo" class="w-6 h-6 rounded-full"/><span>{{ p.symbol }} · {{ p.qty.toFixed(3) }}</span>
      </td>
...
const { data } = useSWRV('positions', fetchPositions, { refreshInterval:60000 })
const positions = computed(()=>data.value||[])
function color(n){ return n>0?'text-emerald-500':n<0?'text-red-500':'' }
</script>
